...
test('BG2A initial board state is deterministic and contains six permanent command seats', () => {
  const options = {
    seed: 424242,
  ...
  assert.deepEqual(first.spaces, {});
  assert.deepEqual(first.pieces, {});
});

test('BG2A authoritative random stream is reproducible and advances explicit RNG state', () => {
...
test('BG2A save serialisation round-trips the exact authoritative state', () => {
...
test('BG2A rejects unsupported actions without mutation or Command Action cost', () => {
...
test('BG2A authoritative state code never uses Math.random', () => {
  for (const file of ['src/game/board-state.ts', 'src/game/board-state-types.ts']) {
