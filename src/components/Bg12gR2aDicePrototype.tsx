...
function addFacePips(group: THREE.Group, face: D6Value, material: THREE.MeshStandardMaterial) {
  const normal = FACE_NORMALS[face];
  const { right, up } = FACE_BASIS[face];
  const pipGeometry = new THREE.CylinderGeometry(0.112, 0.112, 0.026, 28);
  ...
  for (const [column, row] of PIP_LAYOUT[face]) {
    ...
    pip.position
      ...
      .addScaledVector(right, column * 0.47)
      .addScaledVector(up, row * 0.47);
    pip.castShadow = true;
    group.add(pip);
  }
}
...
function makeDie(): THREE.Group {
  ...
  const die = new THREE.Mesh(
    new RoundedBoxGeometry(2, 2, 2, 8, 0.18),
    dieMaterial
  );
  die.castShadow = true;
  ...
  for (const face of D6_VALUES) addFacePips(group, face, pipMaterial);
  return group;
}
...
export function Bg12gR2aDicePrototype({ face }: { face: number }) {
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color(0x171014);
